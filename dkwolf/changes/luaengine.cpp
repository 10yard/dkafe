...
*/
	...
	sol().registry().new_usertype<screen_device>("screen_dev", "new", sol::no_constructor,
			"draw_box", [](screen_device &sdev, float x1, float y1, float x2, float y2, uint32_t bgcolor, uint32_t fgcolor) {
					...
					/*
					10yard - fix bug with rightmost pixels not being written
					...
					*/
					x1 = std::min(std::max(0.0f, x1), float(sc_width)) / float(sc_width);
					y1 = std::min(std::max(0.0f, y1), float(sc_height)) / float(sc_height);
					x2 = std::min(std::max(0.0f, x2), float(sc_width)) / float(sc_width);
					y2 = std::min(std::max(0.0f, y2), float(sc_height)) / float(sc_height);
			...
			"draw_text", [this](screen_device &sdev, sol::object xobj, float y, const char *msg, sol::object color) {
					...
					if(xobj.is<float>())
					{
						...
						*/
						x = std::min(std::max(0.0f, xobj.as<float>()), float(sc_width)) / float(sc_width);
						y = std::min(std::max(0.0f, y), float(sc_height)) / float(sc_height);
					}
